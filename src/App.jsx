...
const LS_MEASUREMENTS = 'sprayer_measurements_v4';
const LS_BLOCKS = 'sprayer_blocks_v4';
const LS_WEATHER = 'sprayer_weather_v2';
const LS_STOCK = 'sprayer_stock_v1';
const AIRMQ_URL = 'https://map.airmq.by/#zoom=10&lat=53.47251789&lng=24.37316895&sidebar=info&selectedLocation=BY042010279';
...
const defaultStock = [
  { id: uid(), name: 'Мерпан, СП', unit: 'кг', available: '0', purchased: '0', used: '0' },
  { id: uid(), name: 'Скор', unit: 'л', available: '0', purchased: '0', used: '0' },
  { id: uid(), name: 'Топаз', unit: 'л', available: '0', purchased: '0', used: '0' },
  { id: uid(), name: 'Строби', unit: 'кг', available: '0', purchased: '0', used: '0' },
  { id: uid(), name: 'Гринда', unit: 'кг', available: '0', purchased: '0', used: '0' },
  { id: uid(), name: 'Волиам Тарго', unit: 'л', available: '0', purchased: '0', used: '0' },
];
...
function weatherCodeToText(code) {
  const map = {
    ...
    45: 'Туман', 48: 'Туман с инеем', 51: 'Слабая морось', 53: 'Морось', 55: 'Сильная морось',
    61: 'Слабый дождь', 63: 'Дождь', 65: 'Сильный дождь', 71: 'Слабый снег', 73: 'Снег',
    75: 'Сильный снег', 80: 'Ливень', 81: 'Ливень', 82: 'Сильный ливень', 95: 'Гроза',
...
function hasStockMovement(item) {
  return [item.available, item.purchased, item.used].some((value) => {
...
export default function App() {
  ...
  const [stockItems, setStockItems] = useState(defaultStock);
  ...
  const emptyBlockForm = {
    id: '', name: '', culture: 'Яблоня', year: '', rows: '', scheme: '4 × 1 м', quantity: '', area: '',
  };
  const [blockForm, setBlockForm] = useState(emptyBlockForm);
  const [editingBlockId, setEditingBlockId] = useState('');

  const emptyStockForm = { id: '', name: '', unit: 'кг', available: '', purchased: '', used: '' };
  ...
  useEffect(() => {
    try {
      ...
      const savedStock = JSON.parse(localStorage.getItem(LS_STOCK) || 'null');
      ...
      if (Array.isArray(savedStock) && savedStock.length) setStockItems(savedStock);
    } catch {
      // ignore broken localStorage
    }
  }, []);

  useEffect(() => { localStorage.setItem(LS_MEASUREMENTS, JSON.stringify(measurements)); }, [measurements]);
  useEffect(() => { localStorage.setItem(LS_BLOCKS, JSON.stringify(blocks)); }, [blocks]);
  useEffect(() => { localStorage.setItem(LS_WEATHER, JSON.stringify(weatherSettings)); }, [weatherSettings]);
  useEffect(() => { localStorage.setItem(LS_STOCK, JSON.stringify(stockItems)); }, [stockItems]);

  const selectedBlock = useMemo(() => blocks.find((item) => item.id === selectedBlockId) || null, [blocks, selectedBlockId]);
  const visibleStockItems = useMemo(() => stockItems.filter(hasStockMovement), [stockItems]);
  ...
  function saveBlock() {
    ...
    const nextBlock = { ...blockForm, name: blockForm.name.trim(), id: editingBlockId || uid() };
    if (editingBlockId) {
  ...
  function saveStockItem() {
    ...
    const nextItem = {
      ...stockForm,
      id: editingStockId || uid(),
      name: stockForm.name.trim(),
      unit: stockForm.unit || 'кг',
      available: stockForm.available || '0',
      purchased: stockForm.purchased || '0',
      used: stockForm.used || '0',
    };
    if (editingStockId) {
      setStockItems((prev) => prev.map((item) => (item.id === editingStockId ? nextItem : item)));
    } else {
  ...
  return (
    <div className="app-shell">
        ...
        {activeView === 'weather' && (
          <section className="card weather-card">
            ...
            <div className="weather-links">
              ...
              <a className="ghost-btn link-btn" href={AIRMQ_URL} target="_blank" rel="noreferrer">Открыть датчик AirMQ</a>
            </div>
      ...
      <aside className={`drawer ${drawerOpen ? 'open' : ''}`}>
        ...
        <div className="drawer-tabs drawer-tabs-3">
          <button className={drawerView === 'measurements' ? 'active' : ''} onClick={() => setDrawerView('measurements')}>Замеры</button>
          <button className={drawerView === 'blocks' ? 'active' : ''} onClick={() => setDrawerView('blocks')}>Участки</button>
          <button className={drawerView === 'stock' ? 'active' : ''} onClick={() => setDrawerView('stock')}>Склад</button>
        </div>
            ...
            <div className="form-grid form-grid-2">
              ...
              <label>
                ...
                <select value={blockForm.culture} onChange={(e) => setBlockForm((prev) => ({ ...prev, culture: e.target.value }))}>
                  <option>Яблоня</option><option>Слива</option><option>Груша</option><option>Черешня</option><option>Другое</option>
                </select>
              </label>
              <label>
                <span>Год</span>
        ...
        {drawerView === 'stock' && (
          <div className="drawer-content">
            <div className="hint">Показываются только позиции, где есть остаток, покупка или расход. Полные нули скрыты.</div>
            <div className="form-grid form-grid-2 top-gap">
              <label className="span-2">
                <span>Препарат</span>
                <input value={stockForm.name} onChange={(e) => setStockForm((prev) => ({ ...prev, name: e.target.value }))} placeholder="Например, Скор" />
              </label>
              <label>
                <span>Ед. изм.</span>
              ...
              <label>
                <span>В наличии</span>
                <input value={stockForm.available} onChange={(e) => setStockForm((prev) => ({ ...prev, available: e.target.value }))} />
              </label>
              <label>
                <span>Куплено</span>
                <input value={stockForm.purchased} onChange={(e) => setStockForm((prev) => ({ ...prev, purchased: e.target.value }))} />
              </label>
              <label>
                <span>Израсходовано</span>
                <input value={stockForm.used} onChange={(e) => setStockForm((prev) => ({ ...prev, used: e.target.value }))} />
              </label>
            </div>
            <div className="action-row">
              <button className="primary-btn" onClick={saveStockItem}>{editingStockId ? 'Сохранить препарат' : 'Добавить препарат'}</button>
              <button className="ghost-btn" onClick={resetStockForm}>Очистить</button>
            </div>
            <div className="blocks-list">
              {visibleStockItems.length === 0 && <div className="block-card">Пока все позиции нулевые. Они скрыты.</div>}
              {visibleStockItems.map((item) => (
                <div className="block-card" key={item.id}>
                  <div className="block-title">{item.name}</div>
                  <div className="block-meta">В наличии: {item.available || '0'} {item.unit} · Куплено: {item.purchased || '0'} {item.unit} · Израсходовано: {item.used || '0'} {item.unit}</div>
                  <div className="action-row small">
                    <button className="ghost-btn" onClick={() => startEditStock(item)}>Изменить</button>
                    <button className="danger-text" onClick={() => removeStockItem(item.id)}>Удалить</button>
                  </div>
                </div>
              ))}
            </div>
